...
const V4_LABELS = { allin: "5-Bet Jam", call: "Call", fold: "Fold" } as const;

const SEAT_NAME: Record<string, string> = { EP1: "UTG", EP2: "UTG+1", EP3: "UTG+2", LJ: "UTG", EP: "EP", IP: "IP" };
const seat = (p: string) => SEAT_NAME[p] ?? p;
const RFI_SHORT: Record<string, string> = { EP1: "UTG (9-max)", EP2: "UTG+1", EP3: "UTG+2", LJ: "UTG / LJ" };

const V3_SHORT: Record<string, string> = {
  v3_LJ_OOP: "UTG vs 3-bet (OOP)",
  v3_LJ_IP: "UTG vs blind 3-bet",
  v3_HJ_OOP: "HJ vs 3-bet (OOP)",
  v3_HJ_IP: "HJ vs blind 3-bet",
  v3_CO_OOP: "CO vs BTN 3-bet",
  v3_CO_IP: "CO vs blind 3-bet",
  v3_BTN_IP: "BTN vs blind 3-bet",
  v3_SB_OOP: "SB vs BB 3-bet",
  v3_EP: "EP vs 3-bet (9-max)",
};
...
const vo = (hero: string, villain: string, raise: string, call: string, notes: string[], format: ChartDef["format"] = "both"): ChartDef => ({
  ...
  title: villain === "EP" ? `${hero === "IP" ? "In position" : hero} vs early-position open (full ring)` : `${seat(hero)} vs ${seat(villain)} open`,
  short: villain === "EP" ? `${seat(hero)} vs EP (9-max)` : `${seat(hero)} vs ${seat(villain)}`,
  kind: "vsOpen",
  format,
  source: "solver",
  hero,
...
const DEFS: ChartDef[] = [
  ...
  rfi(
    ...
    ["Full-ring UTG opens only ~11%: eight players are left to act.", "Big pairs, big broadways and a few suited wheel aces. Offsuit hands below AJ are folds."],
    "9max",
  ),
  rfi(
    "EP2",
    ...
    ["~14% of hands.", "Slightly more suited aces and suited connectors than UTG."],
    "9max",
  ),
  rfi(
    "EP3",
    ...
    ["~16% of hands: the first seat where suited kings and more connectors show up."],
    "9max",
  ),
  rfi(
    "LJ",
    ...
    [
      "~18% of hands. Five players still to act, so your range must hold up when someone wakes up.",
      "Every suited ace is at least a mix: blockers plus nut-flush potential.",
      "Offsuit hands stop at ATo/KJo — dominated offsuit hands bleed money out of position.",
    ],
  ),
  rfi(
    "HJ",
    ...
    ["~23% of hands.", "Every pair and every suited ace now. KTo, QTo and A9o start mixing in."],
  ),
  rfi(
    "CO",
    "Cutoff open",
    ...
    [
      "~29% of hands. Only the button and blinds are left.",
      "You're really stealing from the button: open wide enough that the BTN can't profitably 3-bet you light.",
    ],
  ),
  rfi(
    "BTN",
    ...
    [
      "~45% of hands. Guaranteed position postflop makes almost every suited hand and most offsuit broadways profitable.",
      "The blinds have to defend out of position — that's where your profit comes from.",
    ],
  ),
  rfi(
    "SB",
    "Small blind open (raise-or-fold)",
    ...
    [
      "~45% of hands, raising to 3bb. Only the big blind is left, but they'll have position on you every street.",
      "Solvers also mix in limps here; raise-or-fold is the simpler, near-equal strategy to learn first.",
    ],
  ),
  ...
  vo(
    ...
    ["Versus a tight UTG range, continue with ~10%.", "3-bet the top (QQ+, AK) plus A5s/A4s as blocker bluffs; flat the pairs and suited broadways that play well in position."],
  ),
  vo(
    "CO",
    "LJ",
    ...
    ["Still facing a strong range — offsuit hands like AJo and KQo are mostly folds.", "The button behind you can squeeze, which is why flats stay tight."],
  ),
  vo(
    "CO",
    "HJ",
    ...
    ["Wider than vs UTG: the HJ opens ~23%.", "3-bets get more frequent and more bluffy (suited wheel aces, a few suited connectors)."],
  ),
  vo(
    "BTN",
    "LJ",
    ...
    ["Position lets the button flat a lot more than other seats.", "Pairs and suited hands realize their equity well when you act last."],
  ),
  vo(
    "BTN",
    "HJ",
    ...
    ["~20% continue: roughly 6% 3-bet, 14% flat."],
  ),
  vo(
    "BTN",
    "CO",
    ...
    [
      "The classic steal-vs-steal spot: ~11% 3-bet, ~22% flat.",
      "You have position on the cutoff for the whole hand, which is why flatting is attractive here and much rarer from the blinds.",
    ],
  ),
  vo(
    "SB",
    ...
    ["SB is out of position against everyone: 3-bet or fold is the default.", "A few pairs flat to set-mine, but the big blind behind can squeeze you."],
  ),
  vo(
    "SB",
    "HJ",
    ...
    ["3-bet or fold. Size up to ~4.5x the open because you're out of position."],
  ),
  vo(
    "SB",
    "CO",
    ...
    ["The CO opens ~29%, so your 3-bet range widens to ~13%."],
  ),
  vo(
    "SB",
    "BTN",
    ...
    ["The button opens ~45%, so the SB 3-bets ~18% — lots of suited aces, broadways and suited connectors.", "Flatting is rare: the BB would get to squeeze or see a cheap flop with position on you."],
  ),
  vo(
    "BB",
    "LJ",
    ...
    ["You're getting a great price (about 3.5-to-1), but UTG's range is strong, so offsuit hands shrink fast.", "Suited hands defend much wider than offsuit: they realize equity out of position."],
  ),
  vo(
    "BB",
    "HJ",
    ...
    ["~33% defend. 3-bets are polarized: premiums plus suited blockers and connectors."],
  ),
  vo(
    "BB",
    "CO",
    ...
    ["~48% defend. Nearly every suited hand continues; offsuit hands need high cards or connectivity."],
  ),
  vo(
    "BB",
    "BTN",
    ...
    [
      "The widest defense in poker: ~57% (about 12% 3-bet, 45% call).",
      "You only need ~27% equity to call a 2.5x open, and you close the action.",
      "3-bet a mix of value (JJ+, AK, AQ) and suited blockers/connectors that play well when called.",
    ],
  ),
  vo(
    "BB",
    ...
    ["You have position on the SB for the whole hand — defend very wide.", "3-bet for value more linearly: the SB's range is wide and capped."],
  ),
  ...
  vo(
    ...
    ["Full ring early opens are ~11-16%. Tighten up a lot.", "Set-mine small pairs in position when stacks are deep enough (live games usually are)."],
    "9max",
  ),
  vo(
    "SB",
    ...
    ["Out of position vs a strong range: very tight."],
    "9max",
  ),
  vo(
    "BB",
    ...
    ["You get a price, but early-position ranges crush weak offsuit hands. Defend suited and paired hands."],
    "9max",
  ),
  ...
  v3(
    ...
    ["A later seat 3-bet you and will have position. Continue ~45% of your opens.", "4-bet KK+ and AK, with A5s/A4s as bluffs that block AA/AK."],
  ),
  v3(
    "v3_LJ_IP",
    "LJ",
    "Blinds",
    "UTG open vs blind 3-bet (you're IP)",
    ...
    ["Position lets you flat more: suited broadways and pairs play well in a 3-bet pot IP."],
  ),
  v3(
    "v3_HJ_OOP",
    "HJ",
    "IP 3-bettor",
    "HJ open vs 3-bet (you're OOP)",
    ...
    ["Continue ~42% of your HJ opens. Most offsuit hands fold."],
  ),
  v3(
    "v3_HJ_IP",
    "HJ",
    "Blinds",
    "HJ open vs blind 3-bet (you're IP)",
    ...
    ["In position you can call suited connectors and more pairs."],
  ),
  v3(
    "v3_CO_OOP",
    "CO",
    "BTN",
    "CO open vs button 3-bet (you're OOP)",
    ...
    ["The button 3-bets you wide (~11%), so you defend wider than from UTG."],
  ),
  v3(
    "v3_CO_IP",
    "CO",
    "Blinds",
    "CO open vs blind 3-bet (you're IP)",
    ...
    ["Blind 3-bets are bigger (~11bb) and you'll have position: continue ~50%."],
  ),
  v3(
    "v3_BTN_IP",
    "BTN",
    "Blinds",
    "Button open vs blind 3-bet (you're IP)",
    ...
    ["The blinds 3-bet the button wide, so you defend ~45% of your opens — mostly by calling in position."],
  ),
  v3(
    "v3_SB_OOP",
    "SB",
    "BB",
    "SB open vs big blind 3-bet (you're OOP)",
    ...
    ["Blind-vs-blind 3-bets are wide; defend ~40% of your SB opens despite being OOP."],
  ),
  v3(
    "v3_EP",
    "EP",
    "Any",
    "Early-position open vs 3-bet (full ring)",
    ...
    ["Your range is already strong; continue ~50% but rarely 4-bet bluff."],
    "9max",
  ),
  ...
  {
    id: "v4_IP",
    title: "Your 3-bet (IP) vs 4-bet",
    short: "3-bet IP vs 4-bet",
    kind: "vs4bet",
    format: "both",
    source: "solver",
    hero: "3-bettor",
    ...
    rest: "fold",
    notes: ["At 100bb a 4-bet commits ~25% of your stack. Jam KK+, AK; call QQ-TT and AQs in position; fold your 3-bet bluffs."],
  },
  {
    id: "v4_OOP",
    title: "Your 3-bet (OOP) vs 4-bet",
    short: "3-bet OOP vs 4-bet",
    kind: "vs4bet",
    format: "both",
    source: "solver",
    hero: "3-bettor",
    ...
    rest: "fold",
    notes: ["Out of position, calling 4-bets is worse, so you jam a bit more and flat less."],
  },

  /* ------------------------ Live: vs limpers ------------------------ */
  {
    id: "vl_EARLY_1",
    title: "Facing 1 limper (early/middle seat)",
    short: "Early vs 1 limper",
    kind: "vsLimp",
    format: "both",
    source: "live",
    hero: "EP-HJ",
    ...
    rest: "fold",
    notes: [
      "Live exploit chart. Iso-raise strong hands to play heads-up with the initiative against a weak limper.",
      "Over-limp small pairs and suited connectors: they want cheap multiway flops and implied odds.",
      "Offsuit hands like KJo and ATo play badly multiway — fold them.",
    ],
  },
  {
    id: "vl_LATE_1",
    title: "Facing 1 limper (cutoff/button)",
    short: "Late vs 1 limper",
    kind: "vsLimp",
    format: "both",
    source: "live",
    hero: "CO-BTN",
    ...
    rest: "fold",
    notes: [
      "Live exploit chart. With position, iso-raise wider — you'll often play heads-up against a capped, weak range.",
      "Standard live sizing: 4-5bb plus 1bb per limper (at $1/$2 many pros use $15 + $5 per limper).",
    ],
  },
  {
    id: "vl_MULTI",
    title: "Facing 2+ limpers",
    short: "vs 2+ limpers",
    kind: "vsLimp",
    format: "both",
    source: "live",
    ...
    rest: "fold",
    notes: [
      "Live exploit chart. Multiple limpers means your raise will often be called multiway, so raise for value only and size up.",
      "Hands that make nuts (sets, nut flushes) love multiway pots. Hands that make one pair don't.",
    ],
  },
  {
    id: "vl_SB",
    ...
    rest: "fold",
    notes: [
      "Live exploit chart. Completing costs only half a blind with several players in, so the price is excellent.",
      "Raise only strong hands: you'll be out of position against multiple players.",
    ],
  },
  {
    id: "vl_BB",
    ...
    rest: "check",
    notes: [
      "Live exploit chart. Checking is free, so you never fold.",
      "Raise big (5-6x plus 1x per limper) with value hands; limpers call too much, so don't bluff-raise.",
    ],
  },
];
